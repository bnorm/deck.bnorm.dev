...
public val dev.bnorm.kc24.image.KotlinLogos.`Kotlin one color logo mark rgb`: ImageVector
    get() {
        if (`_kotlin one color logo mark rgb` != null) {
            return `_kotlin one color logo mark rgb`!!
        }
        `_kotlin one color logo mark rgb` = Builder(name = "Kotlin one color logo mark rgb",
...
private var `_kotlin one color logo mark rgb`: ImageVector? = null
